...
| Smoke tests (`tests/smoke/*.lua`) | **79/79** match (byte-identical stdout+stderr+exit) |
...
Geomean slowdown vs PUC Lua: **1.43x** (lower is better; 1.0x = parity).
...
| int_arith | 1.32x |
| lua_calls | 1.32x |
| table_alloc_setmetatable | 1.30x |
| string_loop | 1.23x |
| temp_table_alloc | 1.20x |
| string_concat | 1.09x |

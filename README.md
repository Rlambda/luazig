...
- [ ] Заменить statement-wide `LOADNIL` точным register liveness и очисткой на
  safepoints. **Блокер:** `frame.top` (`reg_top`) обновляется только для
  CALL/vararg, не для обычных инструкций. GC помечает все `frame.regs`
  (строка 12638 в vm.zig). Удаление LOADNIL требует либо точного `reg_top`
  для каждой инструкции (P15.33 dispatch split), либо PUC-подобного
  `traversestack` clearing выше `L->top`. Эксперимент: пропуск LOADNIL даёт
  ~25% improvement на int_arith, но небезопасен для GC (stale pointers в
  freed temporaries помечаются как roots).
...
| Benchmark | PUC Lua | До P15.31 | P15.31 | P15.32b | P15.32c | P15.33 | PUC→P15.33 |
|---|---|---|---|---|---|---|---|
| int_arith | 0.178 | 9.605 | 8.715 | 1.735 | 1.779 | 1.358 | 7.6× |
| global_arith | 0.520 | 15.863 | 14.879 | 7.716 | 8.099 | 7.220 | 13.9× |
| branch_loop | 0.428 | 12.913 | 11.344 | 4.164 | 3.963 | 3.453 | 8.1× |
| lua_calls | 0.094 | 3.523 | 3.427 | 2.719 | 2.642 | 2.553 | 27.2× |
| array_access | 0.045 | 1.315 | 1.155 | 0.426 | 0.368 | 0.338 | 7.5× |
| hash_access | 0.064 | 1.655 | 1.395 | 0.644 | 0.595 | 0.556 | 8.7× |
| temp_table_alloc | 0.030 | 0.224 | 0.223 | 0.154 | 0.147 | 0.147 | 4.9× |
| string_loop | 0.101 | 0.528 | 0.533 | 0.448 | 0.451 | 0.448 | 4.4× |
| coroutine_yield | 0.043 | 0.429 | 0.411 | 0.329 | 0.320 | 0.314 | 7.3× |
| dynamic_load | 0.133 | 1.224 | 1.122 | 0.409 | 0.408 | 0.370 | 2.8× |
| mixed_arith | 0.181 | — | — | — | 1.834 | 1.514 | 8.4× |
| float_arith | 0.181 | — | — | — | 1.812 | 1.384 | 7.6× |
| string_concat | 0.083 | — | — | — | 0.150 | 0.150 | 1.8× |
| metamethod_add | 0.059 | — | — | — | 0.556 | 0.544 | 9.2× |
| field_access | 0.056 | — | — | — | 0.732 | 0.658 | 11.8× |
| comparisons | 0.541 | — | — | — | 7.070 | 5.705 | 10.5× |
...
array_access -11.5%, hash_access -6.7%.
...
- [ ] `pc`, base, top и closure живут в локальных переменных interpreter loop.
  Частично: `pc`, `regs`, `boxed`, `frame_current_line` уже локальные. `base` и
  `frame_cap` — const из frame descriptor. Полный вынос из `BytecodeExecFrame`
  требует分离 self-modifying stack от register file.
...
- [ ] Cold continuation/debug/error paths вынесены из hot function.
...
- [ ] Уменьшить hash `Node` с 56 до 24–32 байт.
- [ ] Уплотнить chain metadata и исключить дублирующие key/tag fields.
- [ ] Специализированные integer и interned-string lookup/insert paths.
- [ ] Не memset’ить полный большой Node при каждом resize.
...
- [ ] ReleaseFast build на Zig 0.16.0;
- [ ] CPU affinity и warmup;
- [ ] медиана минимум семи прогонов;
- [ ] arithmetic/global/branch/call/table/allocation/string/coroutine/load/GC
  workloads;
- [ ] wall time, process CPU, max RSS и opcode count;
- [ ] при доступном современном ядре — `perf stat` counters и `perf record`;
- [ ] JSON/CSV output с toolchain/CPU metadata;
...
1. После P15.31: геометрическое среднее **<8×**.
2. После P15.32–P15.33: геометрическое среднее **<3×**.
3. После P15.34–P15.35: геометрическое среднее **<2×**.
4. Затем отдельные workloads доводятся до parity или небольшого выигрыша.

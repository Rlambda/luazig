...
- [x] ~~SHRI for `x << K` (finishbinexpneg)~~ — реализовано: PUC transforms `x << K` into `x >> (-K)` emitting SHRI (lcode.c:1832 `finishbinexpneg`), because there is no SHL-with-immediate-RHS opcode. Both K and -K must fit sC (range -127..127). MMBINI carries TMS_SHL (original operator) with B field patched to original K (not -K) so `__shl` receives the correct operand. VM SHRI handler peeks at the following MMBINI to dispatch `__shl` (not `__shr`) when the transform was used. `x << 128` (out of sC range) falls back to LOADI + SHL + MMBIN.

...
- [ ] P11.3. Продолжить сближение `heavy.lua` с PUC memory/perf behavior: текущий результат `1048576` всё ещё далеко от ref `8388608` под 128M vmem.
  - Критерий: следующий documented bounded probe объясняет оставшийся gap и/или снижает его без тестовых обходов.
- [ ] P11.4. Продолжить PUC-first perf оптимизацию `nextvar.lua`: найти следующий hot block после tombstone compaction и снизить full-suite runtime без потери parity.
  - Критерий: `nextvar.lua` parity сохраняется, `tools/perf_guard_core.py` проходит, README фиксирует новый bottleneck и результат.
- [ ] P11.5. Выполнить Zig `0.16` porting pass без деградации bundled Zig `0.15.2` supported path.

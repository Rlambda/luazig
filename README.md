...
**Итог P15.38 (a–h):** geomean **4.77× → 3.43×** (-28.1% slowdown).
Цель ≤3× почти достигнута — оставшиеся hotspot'ы: `lua_calls` (6.00×,
call frame overhead), `hash_access` (5.13×, Node 48B vs 24B + tostring),
`temp_table_alloc` (4.42×, table allocation). Дальнейший прогресс требует

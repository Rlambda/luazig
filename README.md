...
- P10 readiness phase закрыт: есть release gate, supported non-musl build path и честный readiness report. Следующий фокус: закрыть оставшиеся blockers до настоящего drop-in статуса (`heavy.lua` OOM/error-object semantics, perf gap, latest Zig port).
...
- [x] P10.1. Разобрать `heavy.lua` без bounded-time маскировки: определить, это runtime memory boundary, performance issue или некорректная OOM-семантика.
  ...
  - Resource profile: при `LZ_HEAVY_VMEM_KB=131072` ref ловит Lua-level `not enough memory` и завершает `OK` на размере `8388608`; zig теперь возвращает Lua-level `not enough memory`, но достигает меньшего размера таблицы и остаётся заметно медленнее ref.
  - Вывод: это не просто bounded-time issue. Текущий blocker — memory/perf gap table representation и allocator strategy: ref доходит до существенно большего table size перед `not enough memory`.
...
- [x] P10.3. Починить host build/toolchain проблему без обязательного `-Dtarget=x86_64-linux-musl`.
  - Исторический результат P10 был superseded фазой P12: текущий supported path — `zig build -Doptimize=Debug` на актуальном system Zig.
  - `Makefile` продолжает принимать `ZIG_BUILD_FLAGS`, но они больше не нужны для обычной сборки.
- [x] P10.4. Ввести release gates: короткий gate, full safe matrix, API lanes, perf guard, known limitations.
  ...
  - По умолчанию gate использует актуальный system Zig (`zig build -Doptimize=Debug`); переопределение: `LUAZIG_ZIG_BUILD_FLAGS='...' tools/release_gate.sh`.
  - Known limitations для интерпретации результата остаются явными: `heavy.lua` resource-heavy timeout и большой perf gap против PUC Lua.
...
- Supported build path зафиксирован для актуального system Zig: `zig build -Doptimize=Debug`.
- Release gate зафиксирован одной командой: `tools/release_gate.sh`.

Что пока не совместимо с production/drop-in статусом:

- `heavy.lua` остаётся blocker: Lua-level OOM object уже сохраняется, но table size до OOM и perf всё ещё заметно отличаются от PUC Lua.
...
- System Zig является целевым compiler; `zig build -Doptimize=Debug` проходит на текущем toolchain.
...
- [x] P11.5. Выполнить Zig `0.16` porting pass: либо добиться build, либо точно классифицировать remaining blockers.
  - Решение по toolchain: дальше таргетим актуальный system Zig (`zig`, сейчас `0.16.0`); совместимость со старым bundled Zig больше не является целью.
...
- [x] P12.6. После successful latest-Zig build прогнать release gate и parity matrix, затем удалить из README устаревшие оговорки про bundled Zig.
  - Инструменты gate переведены на system `zig` по умолчанию (`tools/release_gate.sh`, `tools/api_regression_lane.py`, `tools/run_tests.py`, `tools/testes_matrix.py` и связанные lanes).
  - Добавлена корректная Lua global `arg` для script запуска; это восстановило `files.lua` parity после latest-Zig IO migration.
  - Проверки: `python3 tools/api_regression_lane.py --timeout 180 --testc-timeout 120` OK; targeted parity gate (`nextvar.lua`, `coroutine.lua`, `calls.lua`, `files.lua`, `locals.lua`, `db.lua`, `gc.lua`) OK; safe matrix `33/34 pass parity`, `zig_fail=0`, `both_fail=1` (`heavy.lua` timeout); perf guard OK.

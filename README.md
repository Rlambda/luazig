...
- [ ] P12.2. Перевести `src/util/stdio.zig` и все stdout/stderr call sites на `std.Io.File.Writer`/`std.process.Init.io` без глобального `std.io` compatibility shim.
- [ ] P12.3. Перевести file loading/writing (`Source.loadFile`, `bc_coverage_out`, stdlib file paths) с legacy `std.fs.cwd()`/`std.fs.File` на `std.Io.Dir`/`std.Io.File`.
- [ ] P12.4. Пройти collection API migration: заменить legacy `ArrayListUnmanaged{}`/writer usage на актуальные `.empty` и новые writer/buffer APIs.
- [ ] P12.5. Пройти env/time/fs остатки (`hasEnvVarConstant`, `getEnvVarOwned`, `timestamp`, rename/delete/read APIs) и добиться успешного `zig build -Doptimize=Debug`.

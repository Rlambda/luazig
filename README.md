...
(bc VM после P15.30.1, оптимизации P15.31–P15.38e) геометрическое среднее
замедления от PUC Lua **3.74×** на microbench (см. P15.37 baseline
...
5. После P15.38a–f: геометрическое среднее **≤3×**. ❌ (достигнуто 3.71×)
...
**Итог P15.38 (a–f):** geomean **4.77× → 3.71×** (-22.2% slowdown).
Цель ≤3× почти достигнута — оставшиеся hotspot'ы: `lua_calls` (5.95×,
call frame overhead), `hash_access` (7.70×, Node 48B vs 24B),
`array_access` (6.57×, bounds check + tag dispatch). Дальнейший прогресс
требует variant TKey (P15.39+) и call frame compaction.

...
2. [P15.21 + P15.22: ownership выполнен, continuation ещё нет]
...
3. Yield: перестать сериализовать активные кадры в `SuspendedFrame`; оставить
   thread-owned frame/register/TBC stack на месте и продолжать его тем же loop.
   Для этого сначала нужно представить yielding `__close`, debug hook,
   `pcall`/`xpcall` и metamethod paths как explicit dispatch continuations.

...
### fix: GC never collected short strings (string_intern never swept)

**Problem:** Short strings (≤40 bytes) interned in `string_intern` were never
swept by the GC. They were pinned in the intern table permanently until VM
shutdown. After `for i=1,10000 do local s = tostring(i) end; collectgarbage("collect")`,
~900 KB of short strings remained (91% not reclaimed). Even 5 full GC cycles
did not free them.

**Root cause:** `internStr` (vm.zig:11914) put short strings in
`string_intern.table` but did NOT register them in `gc_objects` (the `allgc`
equivalent). The GC sweep (`gcSweepOne`) never called `string_intern.sweep()`.
In contrast, `long_literals` (also a `StringIntern`) WAS swept in `gcSweepOne`
phase 3 (vm.zig:16331). Long strings (>40 bytes) were registered in `gc_objects`
via `gcRegisterString` and properly swept.

**Fix (PUC-faithful `sweepfinish`):** Added `gcSweepStringIntern` method that
batch-sweeps `string_intern` during `gcSweepOne` phase 3, alongside
`long_literals.sweep()`. Dead short strings (not marked during this cycle) are
removed from `string_intern.table` and freed. Surviving strings have their
marks reset to the current white (PUC `makewhite`). Also:
- Set `gc_marked = current_white & WHITEBITS` on new short strings (PUC
  `luaC_newobj` sets `currentwhite` on all new objects). Without this,
  `gcIsDead(0, current_white)` returned false (0 has no white bit), so dead
  strings were never identified.
- Fixed `gcNoteAlloc` to use `@sizeOf(LuaString) + raw.len` (matching long
  strings) instead of `+ raw.len + 24` (the hashmap overhead was never
  subtracted on free, causing `gc_count_kb` drift).
- Removed incorrect `gc_age = .old` on existing/new short strings in
  generational mode — PUC `internshrstr` returns existing without changing
  GC state.

**Approach note:** Short strings are kept OUT of `gc_objects` (unlike PUC's
`allgc`) for sweep performance: `gcSweepOne` walks `gc_objects` one object per
incremental step, so adding thousands of short strings would slow the sweep.
Instead, `gcSweepStringIntern` batch-sweeps all short strings in a single pass
at the end of the sweep phase (like `long_literals.sweep`). This is a
deviation from PUC's architecture (short strings in `allgc` + `luaS_remove`
in `freeobj`), justified by the incremental sweep performance difference.

**Results:**
- `tostring(i)` × 10000 + `collectgarbage("collect")`: 897 KB leaked → 0 KB.
- Tables, long strings, closures: already properly collected (no change).
- Generational mode: short strings collected during major cycles and explicit
  `collectgarbage("collect")` (minor cycles don't sweep `string_intern`).
- Matrix 30/31, smoke 49/49 — no regressions.
- Perf: no new regression (pre-existing `string_concat`/`string_loop` regression
  unchanged: 7.86s → 7.97s, within noise).

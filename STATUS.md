...
- [x] **Architecture A1 research (COMPLETED): единая GC lifetime/rooting/constructor
  ...
  intrusive `allgc` как единственный lifetime authority; существующие dense
  registries временно только accelerators; безопасная пустая оболочка каждого
  GC-типа; единый synchronous LIFO `RootScope`; один constructor protocol и
  delete list старых registration/root helpers. Findings review-15 включаются
  в архитектурные acceptance criteria: cold `TempRoots.ensure` — A1;
  discarded-result/tail-wrap и VM-global `c_active_closure` — следующий
...
- [x] **P16.50-review-8 correction (CLOSED by review-8)**: завершить C-API
  ...
  continuation; заставить `status_summary` принимать только точный опубликованный
...
- [x] **Parity: errored coroutine не должна исполнять `<close>` до
  ...
  error/unwind lifecycle без test-specific guard и differential-тест обоих
  моментов наблюдения. Это независимый pre-existing BLOCKER, найденный review-7;
  исправлять после correction, не терять как prose-residual. A1.1R4 M32 — ещё
  один симптом того же first-invalid-op (преждевременный вызов closer):
  `__close=coroutine.yield; error("boom")` даёт в PUC на первом resume
  `false, "...boom"`, `dead`, а в luazig — `true, table, "...boom"`,
  `suspended`. Независимый прямой контроль с `__close`-маркером: PUC вызывает
  его только при `coroutine.close` после failed resume, luazig — во время
  resume. PUC `ldo.c:closepaux` → `luaF_close(..., 0)` и
  `lfunc.c:callclosemethod(yy=0)` → `luaD_callnoyield`: отложенный close при
...
- [x] **Parity BLOCKER: один объект в двух TBC-переменных теряет второй
  builtin-yield closer**: валидная форма
  `local a <close> = o; local b <close> = o` с
  `getmetatable(o).__close = coroutine.yield` в PUC Lua 5.5 даёт два
  последовательных успешных yield одного объекта и затем завершает coroutine;
  luazig даёт первый yield, а на втором `resume` завершает coroutine ошибкой
  `metamethod 'close' is nil`. Первая неправильная операция —
  `runCloseMetamethod` распознаёт завершение прошлого closer по
  `valuesEqual(pending_close_builtin_obj, obj)`: следующая отдельная TBC-запись
  того же объекта ошибочно принимается за re-entry уже исполненного closer и
  пропускается, после чего state machine повторно пытается разрешить close на
  неверном continuation edge. PUC хранит прогресс конкретного `tbclist` level /
  CallInfo invocation, а не identity закрываемого TValue. Исправление должно
  привязать builtin-yield completion к конкретной close-continuation/entry и не
  вводить новый глобальный replay authority; проверить bytecode и C-chain,
  одинаковые и разные объекты, два и три alias-entry, normal/error/forced close,
  GC между resume. Найдено A1.1-final как UNCONFIRMED, подтверждено reviewer
  differential; origin — дефект изменённого TBC continuation path, fate
  FIX-NOW. Open-count 23→24.
  CLOSED (TBC-alias correction, bounded): (1) root cause фактического
  наблюдения глубже guard'а: прямой builtin-arm `continueBytecodeClose`
  стадировал callee по th.top НИЖЕ живых TBC-регистров закрываемого фрейма —
  калящий alias-слот (trace: после первого yield второй entry читал значение
  metamethod'а вместо объекта); fix — единый window-raise до стадии обоих
  arm'ов (PUC callclosemethod/luaT_callTM staging-at-top). (2)
  Архитектурно: `Thread.pending_close_builtin/_obj` и object-equality guard
  УДАЛЕНЫ (grep 0); completion identity — per-entry: pop ДО вызова closer
  (PUC poptbclist) в обоих lanes; re-entry продолжает со следующей entry
  через `waiting_builtin_yield` (bytecode) и popped-chain/CClsretState (C);
  новых replay-owner нет. Один объект в N TBC-слотах → closer N раз LIFO.
  Evidence: репро-чанк `true,o`×2 → `true,"done"` dead (= PUC дословно);
  новый differential-сьют tests/c_api/24_tbc_alias (byte-identical vs PUC
  5.5 ltests, RED на pre-fix); differential matrix 8 кейсов (2/3 alias,
  controls, GC между resume, C-lane testc toclose×2) — match; оба
...
- [ ] **Parity BLOCKER: ошибка Lua-closer в yieldable close во время
  ...
  (longjmp мимо region) — оставшиеся marks закрываются на границе
  восстановления (в корутине это `coroutine.close`: verified PUC log после
  failed resume `v2:nil`, после явного `coroutine.close` `v2:nil,v1:e2`);
  luazig bytecode-lane продолжает закрывать eagerly (`v2:nil,v1:e2` сразу).
  Observable divergence close-timing/set валидной Lua-программы (найдено
  differential-кейсом 07 TBC-alias correction; воспроизводится
  byte-identical на pre-fix дереве и в варианте 07b без builtin-yield
  closer — pre-existing к ac3e2b7). Fix-направление: error-escape из
  close-loop обязан прерывать region (PUC luaD_throw мимо luaF_close) и
  оставлять оставшиеся entries (bc_tbc_regs/chain) для границы
...
- [x] **P16.50-review correction (REOPENED→CLOSED by review-7)**: rollback ownership + C-closure upvalue semantics — (a) BLOCKER 1: opClosure count-prefix rollback неверен при смешанных дескрипторах (proxy/new instack/уже-boxed) — exact ownership worklist/bitmap, rollback только созданных этим вызовом Cells в reverse-порядке; закрыть post-commit окно (gcStoreCellValue после commit) — либо provably-infallible через preparation/order, либо полный rollback owner для Closure/tree/register/accounting/register-slot; dispatch-driven mixed-upvalue тест ([proxy, new instack] из реального bytecode; existing-boxed; провал на следующем Cell и на Closure alloc; post-commit barrier failure; byte-exact всё + minor collection + repeated + success); negative copy count-prefix rollback детерминированно ловится; (b) BLOCKER 2: lua_newthread errdefer НЕ работает (?*lua_State ≠ error union) — inner error-union transaction / явный cleanup helper, ABI-wrapper маппит в null ПОСЛЕ cleanup; preserve/restore прежний vm.c_api_thread; тест против реального экспортированного lua_newthread (fail на registry prepare / Thread alloc / handle alloc / parent stack growth; registries/stack/handle/counters/live-set + GC после); (c) BLOCKER 3: registerfuncs алиасит C-closure upvalues (общие Cell-объекты: setupvalue(f1) виден f2) — PUC luaL_setfuncs (lauxlib.c:965-978) копирует VALUES на стек + lua_pushcclosure (lapi.c:609+) свежий CClosure с inline slots; один канонический C-closure конструктор для pushcclosure+registerfuncs с per-closure Cells; убрать неверный LClosure rationale; differential-тест (upvalueid differs; setupvalue A не меняет B; сбор в обоих порядках без leaks; OOM на non-preinterned names + table-growth setfield); (d) error propagation: lua_pushcclosure/lua_pushcfunction/luaL_setfuncs/luaL_newlib catch {} — обследовать защищённый механизм (protected call/throw) и маршрутизировать ЛИБО зафиксировать архитектурный blocker (owner решает); (e) HIGH: testcChargeMemory коммитит total_bytes ДО нативных аллокаций — split check/reserve от accounting commit / точный rollback; тест с активным testc_ctrl + провалы registry reserve/Userdata/uservalues/payload; аудит всех testcChargeMemory-сайтов; (f) cleanup: устаревшие BLOCKED/KNOWN-leak комментарии в тестах, skip fail-индексов 2..4 в pushcclosure matrix, smoke provenance prose (84 файла, 85-й номер — один из 84).
  ...
  и уже усекает корректные результаты (newmetatable реально pushes table+boolean,
  сканер +1; compare/topointer/traceback push по значению, сканер 0; отрицательный
  settop — относительный индекс Lua stack, сканер превращает n<=0 в depth 0;
  unknown commands оставляют depth без fallback к cap) — требуется runtime-dynamic
  builtin result contract (PUC precallC/luaD_poscall двухфазная модель), сканер
  удалить; BLOCKER 2: luaL_newmetatable не устанавливает __name = tname
  (lauxlib.c:317-327), обе реализации (api.State.newmetatable и testC-команда)
  расходятся; BLOCKER 3: canonical provenance расщеплён — current-gate/manifest
...
Residuals (backlog): big.lua both_fail (pre-existing);
...
M29-остаток (мёртвые testhelper-функции). Новый TBC alias BLOCKER вынесен
отдельным открытым пунктом после reviewer differential. Open-count фазы
30→23 (7 закрытий), затем 23→24 (подтверждение residual).
...
(переименован в `tools/perf/historical-activation-id-proof.json`,
artifact_role=historical, provenance-rename P16.50-review-3).
### T8: activation-ledger (current-frame-activation-ledger.json;
теперь `tools/perf/historical-frame-activation-ledger.json`)
...
### P16.50-review-2: blocking correction (2026-09-16)

По ревью P16.50-review (не принята). Коммиты: C = 18a26a5 → C2 = e11a587
→ D = wrapper.

- **BLOCKER 1 — opClosure heap-worklist fallback leak**: порядок владения
  исправлен (fallback аллоцируется ДО cells; каждый аллокацион владеется
  немедленно; старый порядок — cells первым, fallback вторым, errdefer
  после обоих — возвращал OOM с утечкой cells). B1-тест: nups=20 mixed
  proxy/instack, fail 0..14 + success на heap-ветке; заимствованные Cells
  не тронуты ни на одном индексе.
- **BLOCKER 2 — lua_newthread PUC-контракт**: PUC не имеет nullable-провала
  (lua.h:165, lstate.c:273-291 — throw через protected boundary): cleanup в
  luaNewThreadTx завершается, затем OOM идёт через канонический transport
  (cThrow, LUA_ERRMEM) — null-сентинел удалён; T2 переведён на
  inner-transaction seam (pub) с задокументированным контрактом.
- **BLOCKER 3 — cThrow терял статус**: канонический transport
  c_error_value + c_error_status (LUA_ERRRUN=2/LUA_ERRMEM=4, PUC
  luaD_throw ldo.c:125-146); boundary кодирует -1-status (-3/-5);
  callCFunction -5-arm → error.OutOfMemory + api_status=4 (никаких
  конверсий вида ошибки); cPanic: незащищённый throw вызывает c_panicf
  ровно раз, затем terminal panic (PUC g->panic+abort; dead-c_panicf
  residual закрыт); единый транспорт для lua_newthread + 4 void-обёрток;
  nested prev-restore сохранён. B3-тест через реальный callCFunction:
  OOM→OutOfMemory/api_status=4; lua_error→RuntimeError+исходный объект;
  state-usable-after-error.
- **BLOCKER 4 — testc split неполный + tableResize**: 5 забытых коммитов
  добавлены (apiNewThread, builtinCoroutineCreate, оба Closure-конструктора,
  table.concat); каждый check — транзакция (коммит на успехе нативной
  аллокации, размер симметричен free); tableResize new_hash ownership
  (errdefer фри+реверс до установки; hash_installed-flag). B4-матрица
  (6 сайтов, активный ctrl с ненулевым baseline) нашла 4 дополнительных
  production-дефекта, исправленных в C2: double-reversal (gcNoteFree уже
  покрывает оба ledger'а), thread +64 commit/free asymmetry,
  concat-scratch leak (internStr копирует — slice текся на каждом пути),
  concat permanent commit (PUC transient net-zero; реверс только testc-
  ledger — gc_count_kb scratch'ем не заряжался, gcNoteFree дал бы
  −total_len under-drift, найден snapshot'ом).
- **MEDIUM provenance**: api580-ledger и fixed-load-footprint
  перегенерированы (генераторы найдены, оба GREEN); frame-activation-
  ledger — задокументированный исторический артефакт (генератора нет,
  P16.20 T8); proto-ownership-audit/activation-id-proof — тоже
  исторические ( вне scope).
- **Perf**: объявленная сессия на C2-binary 914caffa… → RESULT: OK;
  baseline 66cd438 не тронут. Единый provenance: все canonical на
  e11a587/914caffa (.text 2610905).
- **Батарея**: 16/16 (fmt, unit D/RF 236/236 0 leaks, selftests, noise+7,
  smoke 84/84 + 84/84 --testc, matrix zig_fail=0/both_fail=1, c_api 23+
  DIFF: PASS, api580, gc.lua, diff --check); crash-loop 20/20.
- **Residual** (задокументированы в тестах, кандидаты следующей фазы):
  C-frame leak на OOM-путях callCFunction; api_status не ставится/не
  сбрасывается на lua_error-пути; LUA_MINSTACK-резерв отсутствует.

### P16.50-review-3: finish protected errors + exact allocator accounting (2026-09-16)

По ревью P16.50-review-2 (не принята). Коммиты: C = fa1dbe2 → C2 = 14a1c70
(regression-fix + тесты) → D = wrapper.

- **BLOCKER 1 — OOM C-frame leak**: callCFunction .lua_err-arm = ОДИН
  структурный unwind для всех статусов (YPCALL park для precover /
  обычный popBuiltinCFrame; старый -5-arm возвращал OOM без pop → +1
  frame на каждый failing C-call). Тестовые компенсации (manual
  popBuiltinCFrame, frames0+1) удалены; B3 ассертит frames==baseline +
  repeated-failures + state-usable-after.
- **BLOCKER 2 — статус-коллизия с ThreadSwitch**: BoundaryResult tagged
  union {ok: u32, yield, thread_switch, lua_err: c_int} заменяет magic
  negatives (старое -1-status КОЛЛИДИРОВАЛО с finishCcall's
  -3=ThreadSwitch: Runtime cThrow из continuation → misclassified;
  ERRMEM -5 → generic negative → RuntimeError). C2-фикс: boundary
  декодирует и отрицательные C-ВЕРНУТЫЕ sentinel'ы testC-shim (-1=err,
  -2=yield — внутренняя машина testC прогоняет Zig-ошибки обычным
  возвратом) — coroutine.lua:1093 pcallk-continuation regression от C
  (найдено обязательной матрицей, stash-верифицировано на fa1dbe2).
  OOM устанавливает PUC MEMERRMSG через setOutOfMemoryError с
  VM-lifetime интернированным литералом oom_msg_str (GC-rooted;
  internStrAssume в OOM-пути мог сам OOM-нуть — найдено B3-тестом).
- **BLOCKER 3 — mapVmError терял вид**: kind-preserving mapVmErr(err):
  OOM→OOM, RuntimeError→Runtime; все 27 catch-сайтов api.zig передают
  исходную ошибку; ERRMEM не маскируется в ERRRUN.
- **BLOCKER 4 — api_status lifecycle**: постоянный api_status=4 write
  удалён (PUC хранит статус на errorJmp/catching boundary; running
  state возвращается в LUA_OK). R3: после caught ERRMEM успешный вызов
  → status 0; yield→complete сброс; dead-coroutine latch ERRRUN.
- **BLOCKER 5 — единая учётная граница**: gcNoteAlloc/gcNoteFree пишут
  ОБА ledger'а (gc_count_kb/debt И testc ctrl.total_bytes — PUC
  luaM_realloc_/luaM_free_ parity); все ручные testcCommitMemory
  удалены; string +24 notes удалены; concat scratch = zero-ledger
  transient; thread +64 asymmetry закрыта. B4: каждая failure-index
  restores EXACTLY, lifecycle nets ZERO, success charge = точные
  gcNoteAlloc байты. R4: nups 0/1/3/20 + string hit/miss/long +
  external LSTRFIX/LSTRMEM + header-alloc OOM с falloc-коллбеком
  ровно раз — whole lifecycle nets zero, БЕЗ counter resets.
- **HIGH — panic boundary**: cThrowOn/cPanicOn с ЯВНЫМ throwing state
  (cPanic использовал vm.cur_handle ≠ L экспортированной API; hook
  видит корректный L + error object — msg установлен на throwing
  c_stack). R5: subprocess-тест — atpanic hook печатает PANIC-SEEN +
  msg=not enough memory + hook-L identity, SIGABRT.
- **R2**: пять экспортированных throwing API через реальные
  callCFunction boundary — каждый fail-индекс (object alloc,
  non-preinterned key intern, fresh-table publish): error.OutOfMemory +
  MEMERRMSG + registry invariants + frames==baseline + live-set пуст.
- **PROVENANCE**: три исторических артефакта переименованы в
  historical-* (frame-activation-ledger, proto-ownership-audit,
  activation-id-proof) + artifact_role: historical первым ключом;
  current-api580-ledger и current-fixed-load-footprint перегенерированы
  на HEAD (найдены с устаревшим SHA при финальной проверке) —
  stale current-*: 0.
- **Perf**: объявленная сессия на C2-binary 6da8f000… → RESULT: OK;
  baseline 66cd438 не тронут. Единый provenance: все current-* на
  14a1c70/6da8f000 (.text 2611710).
- **Батарея**: 15/15 (fmt, unit D/RF 241/241 0 leaks, selftests 87,
  noise + 7 neg, smoke 84/84 + 84/84 --testc, matrix zig_fail=0
  (coroutine.lua regression закрыта C2), c_api 23 + DIFF: PASS, api580,
  gc.lua, diff --check); crash-loop 20/20.
- **FINDING-остаток** (задокументирован в тестах R1/R2, кандидаты
  следующей фазы): finishCcall .lua_err безусловный RuntimeError
  маскирует ERRMEM-статус resume; cThrow передаёт cur_handle для 5 API
  (cThrowOn существует, обёртки не все переключены); lua_createtable
  проглатывает OOM; tracebackFrameLabel 1-byte leak;
  builtinCoroutineResume early-return payload leak; callCFunction
  results-dupe OOM окно.

### P16.50-review-4: close protected-error, C-ABI and ownership blockers (2026-09-16)

По ревью P16.50-review-3 (не принята — 6 FINDING-дефектов). Коммиты:
C = fbaf638 → D = wrapper.

- **BLOCKER 1 — continuation ERRMEM статус**: finishCcall .lua_err-arm
  несёт точный PUC статус (4 → error.OutOfMemory; 2/5 → RuntimeError) —
  ERRMEM из resumed yieldk-continuation больше не маскируется в
  LUA_ERRRUN на resume-границе.
- **BLOCKER 2 — throwing L**: имплицитный cThrow(vm, cur_handle)
  УДАЛЁН; все 5 экспортов передают свой L в cThrowOn; R5 сценарий B
  строго hook-L == throwing-L (expected-defect inequality удалён).
- **BLOCKER 3 — lua_createtable**: бросает LUA_ERRMEM (PUC lapi.c);
  R2-probe перевёрнут (OOM+MEMERRMSG, не silent success).
- **BLOCKER 4 — два leak'а**: tracebackFrameLabel intermediate
  владеется/освобождается на каждом выходе; builtinCoroutineResume
  payload-defer вооружён после объявления (completion/yielded
  early-return'ы больше не текут). R1/R3 absorbing-ассерты → 0-leak
  без компенсаций.
- **BLOCKER 5 — results-dupe OOM**: транзакционен (popBuiltinCFrame +
  setOutOfMemoryError до возврата OOM); R2 sweep-компенсация удалена —
  каждый fail-индекс один точный контракт; results-copy residue =
  PUC-correct success-side эффект (teardown владеет).
- **HIGH 1 — LUA_MINSTACK**: reserve 20 слотов на activation-границе
  callCFunction (PUC luaD_precallC → luaD_checkstack); prologue-провал =
  luaD_checkstack OOM → MEMERRMSG; 20 гарантированных push'ей не
  аллоцируют mid-callback (sweep-карты сдвинуты +1).
- **R2 census**: residue-нулевые ассерты только на pre-callback
  fail-индексах; success-side residue (зарегистрированные объекты)
  честно владеется teardown'ом.
- **Perf**: сессия на 7e4ce045… → RESULT: OK; baseline 66cd438 не
  тронут. Provenance: все current-* на fbaf638/7e4ce045 (api580-ledger
  и fixed-load-footprint найдены stale и перегенерированы);
  historical-* не тронуты.
...
(переименован в `tools/perf/historical-proto-ownership-audit.json`,
artifact_role=historical, provenance-rename P16.50-review-3)

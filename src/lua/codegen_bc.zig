    ...
    fn genSet(self: *Codegen, lhs: *const ast.Exp, val_reg: u8, line: u32) Error!void {
        switch (lhs.node) {
            ...
            .Field => |n| {
                ...
                if (self.upvalIdxForName(n.object)) |upval_idx| {
                    const kid = try self.builder.internString(n.name.slice(self.source));
                    try self.emitSetTabUp(upval_idx, kid, val_reg, line);
                    return;
                }
                const obj = try self.genExp(n.object);
                const kid = try self.builder.internString(n.name.slice(self.source));
            ...
            .Index => |n| {
                ...
                // PUC VINDEXUP store: upvalue table + constant string key
                // → SETTABUP (1 opcode) instead of GETUPVAL+LOADK+SETTABLE.
                if (self.upvalIdxForName(n.object)) |upval_idx| {
                    if (key_ed.val == .k_str) {
                        const kid = try self.builder.internString(key_ed.val.k_str);
                        try self.emitSetTabUp(upval_idx, kid, val_reg, line);

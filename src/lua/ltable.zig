...
pub const NodeKeyTag = enum(u8) {
    ...
    int,
    num,
    /// PUC LUA_VSHRSTR: interned short string (pointer-identity match).
    /// PUC's TValue tag distinguishes these from long strings, enabling
    /// luaH_getshortstr's tag+pointer-only chain walk. P16.28 T2: the
    /// classification is paid ONCE at key insertion, not per-chain-node.
    short_string,
    /// PUC LUA_VLNGSTR: long string (content-equality match). Never matches
    /// through the short-identity branch.
    long_string,
    table,
    closure,
    thread,
...
pub const Node = struct {
    ...
    /// Both string node tags (short/long) are live string keys.
    pub fn isStringTag(tag: NodeKeyTag) bool {
        return tag == .short_string or tag == .long_string;
    }
    ...
    pub fn collectableKeyPtr(self: *const Node) ?*anyopaque {
        return switch (self.key_tt) {
            .short_string, .long_string => @ptrCast(self.key_val.string),
    ...
    pub fn getKey(self: *const Node) Value {
        return switch (self.key_tt) {
            ...
            .short_string, .long_string => .{ .String = self.key_val.string },
    ...
    pub fn keyMatches(self: *const Node, key: Value) bool {
        return switch (self.key_tt) {
            ...
            .short_string, .long_string => key == .String and vm.luaStringEq(self.key_val.string, key.String),
    ...
    pub fn setKey(self: *Node, key: Value) void {
        switch (key) {
            ...
            .String => |s| {
                self.key_tt = if (s.isShort()) .short_string else .long_string;
...
pub inline fn nodeLookupStr(nodes: []Node, key: *LuaString) ?*Node {
    ...
    while (true) {
        ...
        if (Node.isStringTag(n.key_tt) and vm.luaStringEq(n.key_val.string, key)) return n;
        n = n.nextNode(nodes) orelse return null;
    }
}
...
pub inline fn nodeLookupShortStrIdentity(nodes: []Node, key: *LuaString) ?*Node {
    ...
    while (true) {
        ...
        //   keyisshrstr: node key tag is short_string (PUC LUA_VSHRSTR)
        //   eqshrstr:    pure pointer identity (a == b)
        // No content comparison. No long-string fallback.
        if (n.key_tt == .short_string and n.key_val.string == key)
            return n;
        n = n.nextNode(nodes) orelse return null;
    }
}

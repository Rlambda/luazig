...
def main() -> int:
    ...
    zig_lua = __import__("pathlib").Path("/tmp/luazig_rf2").resolve()
    ...
    for path in files:
        ...
        if zig_code != 0 and rel == "locals.lua" and zig_code != -11:
            pass
        if rel == "locals.lua":
            import subprocess as _sp, os as _os, json as _json
            with open("/tmp/locals_ctx.json", "w") as f:
                _json.dump({"env": base_env, "cwd": str(tests_dir)}, f)
            for attempt in range(5):
                rr = _sp.run(zig_cmd, cwd=str(tests_dir), env=base_env, stdin=_sp.DEVNULL, stdout=_sp.PIPE, stderr=_sp.STDOUT, text=True, timeout=120)
                with open(f"/tmp/locals_retry_{attempt}.txt", "w") as f:
                    f.write(f"rc={rr.returncode}\n" + rr.stdout[-400:])
        if zig_code != 0 and rel == "locals.lua":
            import subprocess as _sp, os as _os
            # Reproduce under gdb in the SAME parent context
            gdb_cmd = ["gdb", "-batch", "-ex", "run", "-ex", "bt 6", "-ex", "frame 6", "-ex", "frame 2", "-ex", "info locals", "-ex", "p *mt", "-ex", "p mt->gc_marked", "-ex", "p mt->gc_age", "-ex", "p *self->gc_weak_tables.items", "--args"] + zig_cmd
            g = _sp.run(gdb_cmd, cwd=str(tests_dir), env=base_env, capture_output=True, text=True, timeout=180)
            with open("/tmp/locals_gdb.txt", "w") as f:
                f.write(g.stdout[-6000:])
                f.write("\n---STDERR---\n" + g.stderr[-2000:])
        if False and zig_code == -11 and rel == "locals.lua":
            import sys as _sys
            with open("/tmp/locals_crash.txt", "w") as f:
                f.write(f"CMD={zig_cmd}\nENV_KEYS={sorted(k for k in base_env if 'LUA' in k or 'ZIG' in k)}\n--- FULL OUT ---\n{zig_out}")
